...
function Field({
  ...
  return (
    <div className="field">
      ...
      {/* Форма правит глобальный файл, а проектный ложится поверх: без этой
          строки правка выглядела бы успешной и не давала эффекта. */}
      {field.overridden && (
        <p className="field__warn">
          Перекрыто svarog.yaml этой папки — правка здесь не подействует.
        </p>
      )}
    </div>
  );
