...
### 3.2. Что отложено (блоки B–E, не в этом цикле)
...
* **блок B (долгие run'ы)** — автопродолжение хода вместо ручного
  `svarog resume`; идемпотентный restore checkpoint (повторный resume того
  же checkpoint без побочных эффектов);
* **блок C** — Dream как вторая стадия памяти (фоновая консолидация/переосмысление,
  отдельно от curator'а из §1.1);
* **блок D** — cron/heartbeat/триггеры как источник запуска run'ов, помимо
  интерактивного/API-запуска;
* **блок E (hardening/UX)** — boundary note в ошибках policy
  (человекочитаемое объяснение границы отказа), Runtime Context блок
  (структурированный контекст рантайма отдельно от system-промпта),
  tmpfs-маскировка agent-home (сокрытие control-plane путей от процессов
  внутри sandbox сверх текущего mount-разделения ADR-0015 §0.3).

Ни один из этих пунктов не имеет кода в этой ветке — список зафиксирован
здесь, чтобы не потерялся между циклами.

### 3.3. Что отвергнуто (с причинами)
