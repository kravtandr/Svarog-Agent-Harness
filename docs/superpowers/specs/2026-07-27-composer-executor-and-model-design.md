...
- Выбор липкий на чат, живёт в состоянии клиента и уходит явно в каждом
  ...
  Клиент шлёт `provider` и `model` явно в каждом сообщении, а не только
  когда человек их менял: после того как `GET /models` разрешился, оба
  поля в состоянии уже не пустые, и вычислять «человек ничего не менял» —
  лишняя эвристика с шансом ошибиться. Пин к тому, что человек реально
  видел в поле ввода на момент отправки, честнее умолчания.
