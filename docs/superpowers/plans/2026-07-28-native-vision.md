...
**Поправка после финального ревью (находка 1, КРИТИЧНО).** Формулировка
ниже — то, что было в плане изначально, и это баг: она дописывает
user-сообщение с картинкой сразу после КАЖДОГО tool-результата, ещё внутри
цикла, который дренирует `pending_tool_calls` одного и того же
ассистентского хода. Если в ходе два вызова и первый — `read_image` (а он
`is_read_only`, значит конкурентно-безопасен и типичен в паре с любым другим
чтением), user-сообщение врезается МЕЖДУ первым и вторым tool-ответом.
Openai-совместимый контракт требует, чтобы все tool-сообщения одного хода
шли подряд без ничего постороннего между ними — это 400 у реального
провайдера, и `assert_history_valid` его не ловит (счётчик tool_call_id
сходится, порядок он не проверяет). Диагноз усугубляется тем, что
`openai_compatible.py` подсказка про «модель не приняла изображение»
маскирует эту 400 под отказ модели.
...
**Правильно:** буферизовать `ImageRef`/note через весь ход и переносить в
`state.messages` одним куском ПОСЛЕ хода — когда `state.pending_tool_calls`
полностью дренирован, а не после каждого отдельного результата. Добавить в
`LoopState` поле `pending_images: tuple[ChatMessage, ...] = ()` (переживает
checkpoint — resume посреди хода не должен терять уже накопленные картинки;
`checkpoint.py` `to_dict`/`from_dict` расширяются так же терпимо к старым
строкам без ключа, как уже сделано для `images` на `ChatMessage`). В обоих
местах, где добавляется tool-сообщение (одиночный вызов и батч
конкурентно-безопасных вызовов), собранная нота уходит не в `state.messages`
сразу, а в `state.pending_images`; и только когда `while state.
pending_tool_calls:` в `_execute_pending` завершается — то есть весь ход
дренирован, — буфер переносится в историю и очищается. Во втором месте
(батч) вместо `call` — `prepared.call`.
