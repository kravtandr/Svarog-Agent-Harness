...
**Осиротевшие ресурсы (регрессия, симуляция).** Контейнер агента
(`sleep infinity`), relay-sidecar (`serve_forever`) и internal-сеть
переживают контейнер по построению — завершает их только явный `stop()` в
`finally`. Если родитель `svarog` умирает без finally (SIGKILL, OOM), teardown
не вызывается и ресурсы живут дальше. Гарантировать cleanup на SIGKILL нельзя,
поэтому применяется **GC вместо кооперации**: каждый ресурс помечается PID и
boot-token владельца (`svarog-owner-pid`/`svarog-owner-boot`), а старт каждого
внешнего run сначала **подметает** ресурсы с меткой `svarog-agent=1`, чей
PID-владелец мёртв (или boot иной). Критерий безопасен: живой конкурентный run
(его PID жив) не трогается; PID-reuse ведёт максимум к пропуску подметания, но
никогда — к сносу живого. Дополнительно `infra.start()`/`prepare_launch`
перенесены внутрь `try`, чтобы их сбой не осиротил уже поднятые bridge/сеть.
...
**Коллизия affordance памяти (регрессия S4, симуляция).** У Claude Code
есть собственная auto-память (`~/.claude/…/memory/MEMORY.md`), и на интент
«запомни» агент по умолчанию пишет туда своими `Write`/`Read` мимо
`mcp__svarog__remember` — факт уходит из-под Flow A (без git-версий,
governance, immutability sources/). Источник истины по памяти один — Svarog,
поэтому нативная auto-память **выключается на уровне контейнера**
(`CLAUDE_CODE_DISABLE_AUTO_MEMORY=1`, containment tier 1), а инжектируемый
`CLAUDE.md` явно велит писать факты только через `mcp__svarog__remember`
(cooperative tier 2). Enforcement здесь — периметр, а не одна инструкция.
...
* **Дрейф CLI-контрактов:** версия агента в образе sandbox **намеренно не
  ...
  краснеют и адаптер подтягивается. `svarog init` собирает локальные образы
  всех настроенных external executors до записи конфигурации; `svarog doctor`
  дополнительно проверяет наличие/работоспособность CLI в образе.
