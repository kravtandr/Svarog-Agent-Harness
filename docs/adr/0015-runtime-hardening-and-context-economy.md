...
Фаза 0 реализована (feat/adr-0015-phase0). Фазы 1–5 — предложены. **Фаза 0
(безопасность) блокирует остальные** — новые возможности не приземляются, пока
не закрыты host-side escape'ы. Фазы 3–5 — второй эшелон.
...
## Фаза 0 — P0 security hardening (блокирующая)
...
## Фаза 2 — Отложенная загрузка схем tools
...
## Фаза 3 — Child runs вместо in-process subagents
...
| Фаза | Что | Гейт |
|---|---|---|
| **0** | Security: 0.1 traversal → 0.2 git/mount → 0.5 lease → 0.4 config-snapshot → 0.3 раскладка | **блокирует всё** |
| 1 | 1.1 метаданные → 1.2 spill → 1.3 параллель → 1.4 микрокомпакция → 1.5 индекс → 1.6 стагнация | после 0 |
| 2 | Deferred-схемы | 15+ MCP-tools |
| 3 | Child runs (`parent_run_id`) | после 1 |
| 4 | rg-backed coding tools | после 1 |
| 5 | Ops/UX | параллельно, вне ядра |
