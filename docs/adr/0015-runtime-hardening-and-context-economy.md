...
Все фазы (0–6) реализованы. Фаза 2: `mcp.defer_schemas="auto"` — deferred
включается автоматически при 10+ MCP-tools после discovery. Фаза 5 сделана
без OTel-экспорта (пункт исключён решением 2026-07-11). Фаза 6 — точечные
заимствования из разбора HKUDS/nanobot (блок A, спека
`docs/superpowers/specs/2026-07-20-context-turn-reliability-design.md`); FSM
хода, ремонт истории по образцу референса и `ErrorEvent` для инварианта
истории сознательно не введены — обоснование в разделе фазы 6 в конце
документа.
...
### Реализация фазы 2 — заметки о scoping

* «Строка `имя — однострочное назначение` в промпт» реализована
  provider-neutral: сводка незагруженных deferred-tools живёт в **описании
  `load_tool`** (динамическое `definition()`), а не в отдельном
  system-блоке — не требует ни изменений context_builder, ни provider API.
* Deferred — ось промпта, не доступа: tool остаётся исполнимым через
  `registry.get()` (write-ahead resume, policy — как обычно); скрыта только
  его JSON-схема из `definitions()`.
...
## Фаза 2 — Отложенная загрузка схем tools — ✅ СДЕЛАНО (флаг выключен до гейта)
...
| 6 | Надёжность хода (nanobot): 6.1 инвариант истории → 6.2 маркер компакции → 6.3 порядок схем + cached_tokens → 6.4 ремонт аргументов → 6.5 тайминги фаз | независимо от 0–5, после 1 (использует микрокомпакцию/1.4) | ✅ Сделано |
...
## Фаза 6 — заимствования из nanobot — ✅ СДЕЛАНО

Пять точечных находок из разбора HKUDS/nanobot (лицензия MIT, разбор — в
`docs/superpowers/specs/2026-07-20-context-turn-reliability-design.md` и
`docs/reference-analysis.md` §3), плюс один сопутствующий фикс, вскрытый по
ходу реализации. В отличие от фаз 0–5 — не архитектурные слои, а точечные
правки внутри `runtime/loop.py`, `llm/` и `tools/`; ничего исполняемого не
меняется по объёму, добавляется только надёжность существующего хода.

* **6.1 Инвариант истории** — `runtime/history_invariant.py::assert_history_valid`,
  вызывается в `AgentLoop` перед каждым `provider.complete(...)`. Счёт
  ведётся через `collections.Counter` (`announced`/`answered` по
  `tool_call_id`), а не через множества: сервер вправе легально
  переиспользовать один и тот же `tool_call_id` между разными ходами —
  каждый ход даёт свою пару «объявление → результат» для этого id, и при
  сбалансированных парах (число объявлений равно числу результатов на любой
  момент истории) это не нарушение. Множество различить «дважды объявлен и
  дважды отвечен» от «отвечен дважды на одно объявление» не может — отсюда
  именно счётчик, а не set. Нарушение переводит run в `FAILED` с
  человекочитаемой причиной (какое сообщение, какой `tool_call_id`) в
  `Run.error`, видимой в `traces show`; вызов модели не выполняется, история
  не правится. Таблица `ErrorEvent` для этого сознательно не используется —
  у неё сегодня нет ни одного писателя, и заводить первый ради единственного
  случая было бы расширением объёма задачи. Ремонт истории (orphan/backfill)
  по образцу nanobot (`context_governance.py`) не переносится: write-ahead
  (`LoopState.pending_tool_calls` в checkpoint до исполнения, ADR-0005)
  делает orphan tool-результаты структурно невозможными — вместо ремонта,
  маскирующего будущие баги loop'а, нужна проверка, которая их вскрывает.
  Reproducer'ы: `tests/test_history_invariant.py`,
  `tests/test_resume.py::test_history_invariant_holds_on_resume_after_approval`
  (легальный путь resume-после-approval не задевает инвариант).

* **6.2 Actionable-маркер микрокомпакции** — `runtime/loop.py::_microcompact`:
  текст маркера при отсутствии spill-файла заменён с «повтори вызов при
  необходимости» (провоцирует буквальный повтор того же запроса с тем же
  результатом) на «повтори вызов с более узкими параметрами (путь, паттерн,
  лимит), а не тот же самый»; со spill-файлом маркер отсылает читать его
  `read_file`-частями (offset/limit). Reproducer:
  `tests/test_loop.py::test_microcompact_marker_is_actionable`.

* **6.3 Стабильный префикс схем + cached_tokens** — `tools/registry.py`:
  `ToolRegistry.register()` получил keyword-only `external: bool = False`;
  `definitions()` вместо сплошной алфавитной сортировки отдаёт встроенные →
  внешние (MCP) → `load_tool`. `load_tool` стоит последним в списке
  намеренно: его `description` несёт динамическую сводку незагруженных
  deferred-схем и меняется при каждой загрузке (фаза 2 выше) — в алфавитной
  позиции эта мутация сдвигала бы весь последующий префикс и обнуляла
  prefix-cache провайдера при каждой загрузке; в конце списка она ничего не
  трогает. `load_tool` также явно исключён из группы `external`
  (`n != _LOAD_TOOL_NAME` в обоих списках `definitions()`), чтобы он не мог
  попасть в результат дважды, если его когда-нибудь зарегистрируют с
  `external=True`. Отдельно — `llm/provider.py::Usage.cached_tokens` и
  `llm/openai_compatible.py::_cached_tokens` разбирают три диалекта usage
  (`prompt_tokens_details.cached_tokens`; верхнеуровневый `cached_tokens`;
  `prompt_cache_hit_tokens`). `cached_tokens` — подмножество `prompt_tokens`,
  а не добавка к нему: `total_tokens` и расчёт стоимости его не учитывают;
  счётчик накапливается отдельно в `LoopState.cached_tokens` (переживает
  resume через checkpoint) → `Run.meta["cached_tokens"]` → `traces show`.
  Без этой метрики эффект стабилизации префикса был бы нечем измерить.
  Reproducer'ы: `tests/test_deferred_tools.py` (стабильность встроенного
  префикса при добавлении MCP-tool, порядок `load_tool`, round-trip
  `cached_tokens` в checkpoint), `tests/test_llm.py` (три диалекта usage),
  `tests/test_loop.py` (накопление `cached_tokens` за несколько итераций
  хода).

* **6.4 Ремонт формы tool-вызова** — `tools/registry.py::ToolRegistry.prepare_arguments`
  чинит двойную JSON-сериализацию (`double_encoded`: `arguments_json`
  распарсился в строку, которая сама — JSON-объект) и конверт
  `{"arguments": {...}}` (`unwrapped`; ремонт применяется только если у
  `tool.args_model` нет собственного параметра `arguments` — иначе развернуть
  значило бы сломать легальный вызов). Строгий разбор делегирован
  существующему `ToolCallRequest.parse_arguments`: ремонт запускается только
  при его провале (`ValueError`), не подменяет его — так у метода остаётся
  вызывающий в продакшен-коде, и он не превращается в мёртвый код после
  введения ремонта. `UnknownToolError` получил подсказку через
  `difflib.get_close_matches` по нормализованному имени; подсказка — только
  текст сообщения, неизвестное имя никогда не исполняется. Факт и содержание
  ремонта пишутся в trace (`ToolCall.arguments["_repairs"]`, `"_raw"` —
  исходная строка после redaction), в т.ч. на пути параллельного батча
  (§1.3). Reproducer'ы: `tests/test_tools_base.py`,
  `tests/test_loop.py::test_repaired_call_records_original_in_trace`.

* **6.5 Тайминги фаз хода** — `runtime/phase_timer.py::PhaseTimer`
  (контекст-менеджер `measure(phase)`, снимок `as_meta()`, восстановление
  ...
  у Svarog вход в ход один, управление — линейный `while` с фазами, поэтому
  таблица переходов не вводится — ценность нужна не в диспетчере состояний,
  а в ответе «где встал run». Фазы вложены друг в друга (`memory_flush` и
  `checkpoint` считаются внутри `tool_exec`, симметрично и в основном цикле,
  и в write-ahead ветке `_drive` до входа в цикл) — агрегат представляет
  собой набор **пересекающихся срезов** времени хода, а не partition: сумму
  по фазам нельзя читать как 100% длительности итерации. `approval_wait`
  измеряет только создание approval-записи и checkpoint перед уходом в
  `waiting_approval` — собственно ожидание решения человека происходит
  **между** run'ами (`resume` — отдельный вызов/процесс) и этим таймером не
  измеряется вовсе. Reproducer'ы: `tests/test_phase_timer.py`,
  `tests/test_loop.py::test_phase_timings_land_in_run_meta`,
  `tests/test_resume.py::test_suspended_run_resumes_and_completes` (агрегат
  переживает suspend+resume), `tests/test_resume.py::test_resume_survives_malformed_phase_meta`
  (испорченный `meta["phases"]` не роняет resume).

* **Побочный фикс: пред-существующая гонка read-modify-write в trace-слое** —
  запись `Run.meta["phases"]` на каждой итерации (а не изредка, как раньше
  `cached_tokens`) превратила редкий путь в горячий и вскрыла
  пред-существующую гонку в `trace/recorder.py::merge_run_meta` /
  `update_progress`: read-modify-write шёл по **локальной** (в памяти сессии
  loop'а) копии `run.meta`, и запись `run.meta = {**run.meta, **extra}`
  затирала `cancel_requested`, выставленный из другой сессии БД (например,
  HTTP-запрос `/runs/{id}/cancel`) в промежутке между чтением и коммитом —
  флаг кооперативной отмены молча терялся. Исправлено добавлением
...
### Отвергнутые варианты фазы 6 (зафиксировано, чтобы не пересматривалось)

* **Ремонт истории (orphan/backfill) по образцу nanobot** — не переносится:
  write-ahead делает orphan tool-результаты структурно невозможными у
  Svarog; вместо ремонта — проверка инварианта (§6.1), которая вскрывает
  баги loop'а, а не маскирует их.
* **Конечный автомат с таблицей переходов для хода** — не вводится: у
  Svarog один вход в ход (линейный `while`); множественность входов у
  nanobot — их главный довод в пользу FSM — здесь не воспроизводится (§6.5).
* **`ErrorEvent` для нарушения инварианта истории** — не используется: у
  таблицы нет ни одного писателя сегодня, и заводить первый ради
  единственного случая — не оправданное расширение объёма задачи (§6.1).
* **Предварительная оценка токенов через tiktoken и цепочки резервных
  провайдеров** — исключены из объёма решением 2026-07-20 (см. спеку блока
  A, раздел «Цель»); не относятся к надёжности хода как таковой.
