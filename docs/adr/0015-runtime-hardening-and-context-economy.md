...
Фазы 1–5 — предложены, не начаты. **Фаза 0 (безопасность) блокирует остальные**
— новые возможности не приземляются, пока не закрыты host-side escape'ы.
Фазы 3–5 — второй эшелон.
...
| Фаза 1 — экономика контекста (1.1–1.6) | ❌ Не начато |
...
## Фаза 1 — Экономика контекста — ❌ НЕ НАЧАТО
...
**Реально в схеме (фаза 0):** только `sandbox.separate_git_dir`. Остальные поля
блока (`runtime.microcompact_*`, `tool_output_context_chars`,
`max_tool_concurrency`, `stagnation_repeats`, `memory.index_max_lines`,
`mcp.defer_schemas`) — из фаз 1–2 и в `SvarogConfig` ещё НЕ добавлены; они
приземляются вместе со своими фазами.
...
| 1 | 1.1 метаданные → 1.2 spill → 1.3 параллель → 1.4 микрокомпакция → 1.5 индекс → 1.6 стагнация | после 0 | ❌ Не начато |
