...
Перед каждым вызовом модели рантайм проверяет инвариант согласованности истории диалога (первое сообщение — system; у каждого объявленного tool-вызова ровно один результат) и не отправляет модели испорченную историю — нарушение сразу переводит run в FAILED с человекочитаемой причиной, а не подделывает историю заглушками. Маркер микрокомпакции формулируется как actionable-инструкция («повтори с более узкими параметрами»), а не как приглашение повторить тот же самый запрос. Подробности и обоснование — ADR-0015 §6.1–6.2.
...
* запуск verifier;
* обработка ошибок;
* проверка инварианта истории диалога перед каждым вызовом модели;
* сохранение trace;
* измерение длительности фаз хода (вызов модели, исполнение tools, checkpoint, ожидание approval) для диагностики, где встал run;
...
Тайминги фаз хранятся как набор пересекающихся срезов времени (фазы вложены друг в друга, например checkpoint — внутри исполнения tools), а не как разбиение итерации на 100%; фаза ожидания approval измеряет только создание approval-записи, а не время, которое человек реально тратит на решение (оно проходит между run'ами и не измеряется). Подробности — ADR-0015 §6.5.
...
Порядок инструментов в схеме, отправляемой модели, стабилен: встроенные инструменты (отсортированы по имени) → внешние MCP-инструменты (отсортированы по имени) → служебный `load_tool` последним. Так изменение состава MCP-инструментов или загрузка отложенной (deferred) схемы не сдвигает уже закэшированный провайдером префикс промпта. Искажённая форма аргументов tool-вызова (двойная JSON-сериализация, лишний конверт вида `{"arguments": {...}}`) распознаётся и чинится Tool Router'ом, а не отклоняется как ошибка модели; строгий разбор при этом не подменяется — ремонт запускается только когда он проваливается. Факт и содержание ремонта фиксируются в trace, чтобы аудит видел и то, что прислала модель, и то, что реально исполнилось. Подробности — ADR-0015 §6.3–6.4.
...
* автопродолжение: при `runtime.max_refuel_rounds > 0` run пересобирает
  контекст и продолжает работу сам, не требуя `svarog resume`; потолок
  раундов и бюджеты токенов/стоимости — общие стоп-краны, а
  `max_iterations` ограничивает сегмент между refuel'ами (ADR-0005);
...
`token usage` включает `cached_tokens` — число токенов промпта, обслуженных из кэша провайдера. Это подмножество токенов промпта, а не добавка к ним: общий счётчик токенов и расчёт стоимости `cached_tokens` не увеличивают, это отдельная метрика эффективности. `duration` детализируется по фазам хода (вызов модели, исполнение tools, checkpoint, ожидание approval и т.д.); фазы вложены друг в друга, поэтому сумма длительностей по фазам не равна 100% времени итерации.
...
На TTY `svarog chat` работает в inline-режиме (ADR-0018): диалог живёт в обычном буфере терминала (scrollback, нативное выделение и копирование — без alt-screen), стрим текущего ответа и прогресс (итерации/токены/$/контекст) — в живой нижней области, финальный ответ печатается отрендеренным markdown'ом. Презентация как у Claude Code/qwen-code: welcome с логотипом Svarog (синий акцент), мини-таблица доступных executors / local·cloud / policy profile (активное подсвечено), компактный статус над промптом; меню `/`-команд и `@`-файлов появляется только при наборе (prompt_toolkit, CompletionMode). Tool-вызовы — краткие карточки (путь/команда/размер — без дампа `content`). Approval и ask_user решаются прямо в чате, слэш-команды `/help /new /sessions /fork /executor /mode /policies /copy /quit` (`/executor` `/mode` `/policies` — RadioList ↑↓+Enter, выбор пишется в project `svarog.yaml` и пересобирает sandbox), Ctrl+C во время run — прерывание (suspended). `--plain` или отсутствие терминала — построчный REPL.

`run`/`chat`/`resume` запускаются из любой папки: если workspace пересекается с control-plane Svarog (БД/память/скиллы — ADR-0015 §0.3), локальный TTY-запуск показывает пересечения и просит явное подтверждение («агент сможет менять код/настройки самого Svarog»); без TTY, в `--json`, в gateway и для standard-тенантов гейт остаётся fail-closed без подтверждения.
...
* секреты хранятся в secret store (pluggable backend: MVP — JSON-файл `0600` без шифрования + env fallback; далее — шифрованный файл, внешний vault);
...
runtime:
  ...
  max_refuel_rounds: 12       # 0 — приостановка вместо автопродолжения
...
Policy-профиль проекта можно настроить интерактивно командой
`svarog policies configure` (или сразу выбрать его через `--profile`). Команда
редактирует только `policies.profiles` в project-level `svarog.yaml`;
неотключаемый critical-набор по-прежнему нельзя ослабить конфигурацией.
