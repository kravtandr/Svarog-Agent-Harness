...
На TTY `svarog chat` работает в inline-режиме (ADR-0018): диалог живёт в обычном буфере терминала (scrollback, нативное выделение и копирование — без alt-screen), стрим текущего ответа и прогресс (итерации/токены/$/контекст) — в живой нижней области, финальный ответ печатается отрендеренным markdown'ом. История ввода стрелками (readline), approval и ask_user решаются прямо в чате (включая resume-цикл native-пути), слэш-команды `/help /new /sessions /fork /copy /quit`, Ctrl+C во время run — прерывание (suspended). `--plain` или отсутствие терминала — построчный REPL.
