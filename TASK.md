...
На TTY `svarog chat` работает в inline-режиме (ADR-0018): диалог живёт в обычном буфере терминала (scrollback, нативное выделение и копирование — без alt-screen), стрим текущего ответа и прогресс (итерации/токены/$/контекст) — в живой нижней области, финальный ответ печатается отрендеренным markdown'ом. Презентация как у Claude Code/qwen-code: welcome с логотипом Svarog (синий акцент), тип executor (`native/local` или `external/claude-code`…), полоса над промптом; меню `/`-команд и `@`-файлов появляется только при наборе (prompt_toolkit, CompletionMode). Tool-вызовы — краткие карточки (путь/команда/размер — без дампа `content`). Approval и ask_user решаются прямо в чате, слэш-команды `/help /new /sessions /fork /copy /quit`, Ctrl+C во время run — прерывание (suspended). `--plain` или отсутствие терминала — построчный REPL.
