...
На TTY `svarog chat` открывает полноэкранный TUI (ADR-0018): markdown-транскрипт со стримингом токенов, история ввода стрелками, статус-бар прогресса (итерации/токены/$/контекст), approval- и ask_user-модалки на месте (включая resume-цикл native-пути), слэш-команды (`/help /new /sessions /fork /quit`) с автодополнением, session picker с превью, сворачиваемая панель tool-calls/событий (`Ctrl+T`), Esc — прерывание текущего run (suspended). `--plain` или отсутствие терминала — построчный REPL.
