...
class GitRepo:
    ...
    async def add_all(self) -> None:
        """Застейджить изменения ЭТОГО каталога.

        Pathspec `-- .` обязателен: `git add -A` без него стейджит всё рабочее
        дерево репозитория, а не текущий каталог. Для пути, лежащего внутри
        чужого репозитория (каталог памяти внутри чекаута проекта), это
        утаскивало в коммит незакоммиченную работу человека. Из корня
        репозитория поведение не меняется. Кампания 23.07.2026.
        """
