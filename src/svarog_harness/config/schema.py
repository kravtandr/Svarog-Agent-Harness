...
class DreamConfig(StrictModel):
    ...
    Идёт на `models.auxiliary`. Качество консолидации требовательно к модели:
    по эмпирике S33 (ADR-0021 «Пригодность моделей») deepseek-chat пропускает
    противоречия внутри секции профиля, claude-sonnet-4.5 — справляется. Для
    Dream указывай способную auxiliary-модель, не самую дешёвую.
    """
