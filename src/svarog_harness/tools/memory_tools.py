...
class SearchMemoryTool(Tool[SearchMemoryArgs]):
    ...
    description = (
        ...
        "index.md нужную страницу не найти. Запрос — ключевые слова, не вопрос "
        "целиком. Поиск не знает морфологии: если ничего не найдено, повтори "
        "запрос корнем слова или синонимом ('счета' → 'счет', 'платежи')."
    )
    risk_level = RiskLevel.LOW
