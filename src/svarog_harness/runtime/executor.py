...
# (кампания 21.07.2026, S11: brainstorming «съедал» деливерабл на обоих
# адаптерах). Имя tool у адаптеров разное (mcp__svarog__ask_user /
# svarog_ask_user) — подставляется параметром.
def ask_user_guide(tool_name: str) -> str:
    return (
        "# Вопросы человеку\n\n"
        f"Нужен ответ или решение человека — вызывай MCP-tool `{tool_name}` и "
        "жди результата. НИКОГДА не завершай работу текстом-вопросом: в "
        "headless-режиме на него некому ответить, и задача будет засчитана "
        "проваленной. Для творческих задач предпочитай разумные дефолты; "
        f"`{tool_name}` — только когда выбор реально блокирует работу. Если у "
        "вопроса есть 2–5 чётких вариантов — передай их параметром `options`: "
        "человек выберет один стрелочками.\n\n"
        "Так же НИКОГДА не завершай ход анонсом будущей работы («сейчас "
        "сделаю…», «разработаю и сохраню…»): ход без вызова tools — это "
        "финальный ответ, после него работу продолжить нельзя. Скиллы "
        "(brainstorming, writing-plans и т.п.) не отменяют это правило: "
        "деливерабл обязан быть готов в этом же запуске."
    )


def memory_scope_guide() -> str:
    """Память общая для всех рабочих папок и не описывает текущую.

    Трейс 06.08.2026: в пустой папке `test` агент нашёл в поданном целиком
    index.md единственный знакомый проект и выдал его за текущий. Индекс —
    каталог всего, что Svarog помнит, а не опись рабочей папки; правило нужно
    и нативному циклу, и внешним агентам, поэтому живёт общим хинтом.
    """
    return (
        "# Память и текущая папка\n\n"
        "Память общая для всех рабочих папок и НЕ описывает текущую. Страница "
        "проекта в index.md — про тот проект, а не про то, что лежит в твоей "
        "рабочей папке сейчас; у части страниц папка указана прямо в индексе. "
        "О содержимом текущей папки суди по самой папке. Пусто — так и скажи "
        "«здесь пусто» и спроси, что делать; не выдавай знакомый по памяти "
        "проект за текущий."
    )


def memory_sources_guide(remember_tool: str) -> str:
    """Разводка «чужой исходник → память sources/» vs «твой деливерабл → workspace».

    Регрессия S3 (прогон 30.07.2026, glm-5.2/opencode): паттерн «результат →
    нативный write» перебивал memory-гайд для лексики «спека/исходник» — агент
    писал чужую спеку в файл workspace вместо памяти. Native-промт разводит эти
    назначения с фикса 5bd759f (_MEMORY_GUIDE); внешним агентам то же правило
    доставляется этим хинтом.
    """
    return (
        "Разводи назначения записи. Нативные write/edit — ТОЛЬКО для твоего "
        "деливерабла: документа, кода, конфига, который ты САМ сочинил по "
        "задаче. ЧУЖОЙ материал, который просят сохранить («сохрани эту "
        "спеку/исходник/доку как…», «запомни этот текст»), — это память, а не "
        f"workspace: вызывай `{remember_tool}` operation=create с путём "
        "sources/<slug>/… и ссылайся на него со страницы проекта. Сохранённые "
        "sources/* неизменяемы: не редактируй их ни через память, ни созданием "
        "одноимённой «поправленной» копии в workspace — если просят исправить "
        "исходник, откажи и объясни неизменяемость."
    )


def competency_honesty_guide() -> str:
    """Правило честности про пробелы в компетенциях/ресурсах (S26).

    Детектор галлюцинации из симуляции S26: модель назначала ближайшего
    стек-соседа (React/TS-фронтендера) на «мобильную разработку на React
    Native», не оговорив, что мобильного разработчика в команде нет и
    React ≠ React Native. Правило заставляет ЯВНО отмечать пробел, а не
    маскировать его назначением.
    """
    return (
        "# Честность про пробелы в компетенциях и ресурсах\n\n"
        "Назначая людей, инструменты или подходы под задачу, отмечай ЯВНО, "
        "когда нужной компетенции среди имеющихся НЕТ. Не подставляй "
        "ближайшего стек-соседа без оговорки: React ≠ mobile-разработка (без "
        "React Native), frontend ≠ backend, знание библиотеки ≠ продакшен-"
        "опыт. Если в команде/наборе нет того, кого задача требует по-"
        "настоящему, — назови эту дыру прямо и предложи реальный вариант "
        "(нанять, аутсорс, дообучить, сменить технологию), а не маскируй её "
        "назначением «подойдёт, потому что похоже». Заказчик должен видеть "
        "риск, чтобы принять осознанное решение, а не получить скрытый "
        "провал."
    )
...
class AgentAdapter(Protocol):
    ...
    def context_files(
        ...
        моста (spec 2026-07-24). Адаптер называет tools в своём неймспейсе;
