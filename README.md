...
* **Изоляция арендаторов.** Один хост обслуживает много пользователей, у каждого — **свой agent-home** (память, скиллы, секреты, БД, workspace), между которыми нет доступа. Роли **superuser** (доступ к хосту) и **standard** (принудительно заперт в docker-sandbox, fail-closed без docker) плюс per-tenant auth и квоты. Это не «pairing в личке», а полноценная многоарендность на уровне enforcement.
...
* **Регрессии агента ловятся симуляцией.** В репозитории — **agent-based user simulation** (`simulation/`): внешний агент играет пользователя по сценариям и личностям, гоняет реального Svarog на реальном LLM и проверяет траекторию (какие tools вызваны, не зациклился ли, попал ли результат в файл/память). Это систематическая проверка поведения самого агента, а не только unit-тесты кода.
...
* **Интерфейсы**: CLI (`run`, `chat`, `resume`, `traces`, `approvals`, `skills`, `memory`, `secrets`, `tenant`), REST + WebSocket API, Telegram-бот — с асинхронным approval.
...
Gateway (`svarog-harness[server]`, §10.4) поднимает тот же прогон задачи через HTTP: `POST /runs` создаёт run и сразу возвращает `run_id`, `WS /runs/{id}/events` стримит текст/tool calls/checks/финал, `POST /approvals/{id}` принимает решение (approve/deny), а `POST /approvals/{id}/answer` — текстовый ответ на вопрос `ask_user`; оба асинхронно возобновляют run (ADR-0005). CLI и gateway используют один `TaskRunner`, поэтому логика агента не дублируется. В долгоживущем процессе gateway работает refuel-supervisor (секция `supervisor`): refuel-suspended run'ы поднимаются автоматически, без ручного `resume`. При bind не на loopback (`--host 0.0.0.0`) обязателен bearer-token: укажите `gateway.token_ref` и сохраните значение в SecretStore; при заданном токене REST/WS требуют `Authorization: Bearer …` или `?token=…` для WebSocket. При `tenancy.enabled` gateway работает **мультиарендно** (ADR-0014): bearer-токен — или JWT, если задан `tenancy.jwt_secret_ref` — резолвится в тенанта через реестр, и каждый run исполняется в изоляции своего agent-home; роль `standard` принудительно заперта в docker-sandbox, а квоты тенанта дают HTTP 429.

Telegram-бот (§10.2) — тот же `GatewayService` поверх Bot API: сообщение порождает run, ход прогона идёт в чат, `waiting_approval` показывается inline-кнопками approve/deny, а вопрос `ask_user` — текстом: следующее сообщение в чат становится ответом (или `/skip` — продолжить без ответа). Бот тоже гоняет refuel-supervisor, поэтому долгие автономные задачи продолжаются сами. Токен бота — секрет (`telegram.token_ref` → SecretStore, ADR-0006), доступ ограничен allowlist'ом `telegram.allowed_users` (§16). В мультиарендном режиме (`tenancy.enabled`) доступ определяется реестром (`telegram:<user_id>` → тенант), а не плоским allowlist'ом, и каждый пользователь работает в своём agent-home; при `provisioning: first_touch` новый пользователь авто-провижнится в отдельного тенанта.
