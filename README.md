...
* **Web UI** (§10.3) — веб-клиент «Горн» сделан поверх REST/WS gateway: чат с выбором исполнителя/провайдера/модели, настройки, память, скиллы, трейс и дифф запусков (`docs/superpowers/specs/2026-07-27-web-ui-gorn-design.md`); остаются approval inbox отдельным экраном, skill proposals/curate и светлая тема. «Человеческие» UI сегодня — CLI, Telegram и веб.
